...
class onlineTimeline: 
    def __init__(self,inputfile,rowNum,outputfile):
    ...
    def reserveRoute(self,route):
        ...
        self.removeLinks(self.network,route3)
        for rt in route:
            if(rt[1] == 'E'):
    ...
    def removeLinks(self,network,route):
        for i in range(0,len(route)-1):
    ...
    def addLinks(self,network,route):
        for i in range(0,len(route)-1):
        ...
        for i in sendThisTime:
            ...
            for dest in self.taskGraph[str(i)]['out_links'][:]:
                ...
                if(self.outLinks[linkName]["state"]==-1):
                    ...
                    defaultRoute = self.computeRouteXY(srcMapto,dstMapto)
                    #print("compute route",self.changeIndex(srcMapto),self.changeIndex(dstMapto))
                    route,canSend  = self.computeRouteOnline(self.network,self.changeIndex(srcMapto),self.changeIndex(dstMapto))
                    ...
                    if(canSend!=0):
                        ...
                        dest[0][3] = defaultRoute
                    # if(i==242):
                    #     print(srcMapto,sendtoi,dstMapto)
                    #     print(dest[0][3],defaultRoute,canSend)
                    #     print(dest)
                   ...
                   # print("dijkstra_path: ",route, "  defaultRoute: ",defaultRoute)
                    ...
                    if(canSend==0):
                        ...
                        self.pendTask.append([i,sendtoi,defaultRoute])
                        self.nowPri=1000
    ...
    def releaseRec(self,releaseList):
        ...
        for item in releaseList:
            if(item!=None):
                ...
                self.addLinks(self.network,route1)
    ...
    def computeTime(self):
        ...
        with open("result.txt","a") as t:
            t.write(self.outputfile)
            t.write("\n Total time: "+str(self.nowTime)+"\n")
        # self.Dprint("sendMatric",self.sendMatrix)
        # self.Dprint("receiveMatric",self.receiveMatrix)
        # self.Dprint("exeMatric",self.exeMatric)
        # self.Dprint1("sendingNow",self.sendingNow)
...
def main(argv):
    ...
    try:
        ...
        print('Error RoutingAlgorithm1.py -i <inputfile> -r <row>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('RoutingAlgorithm1.py -i <inputfile> -r <row>')
            sys.exit()
        elif opt in ("-i", "--ifile"):
    ...
    task = onlineTimeline(inputfile,rowNum,outputfile)
    task.loadGraph()
    task.computeTime()
